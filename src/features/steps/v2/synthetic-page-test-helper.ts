...
import type { StepDetectorV2Region } from "./contracts"
...
export function createSyntheticV2Page(
...
function containsPixel(region: StepDetectorV2Region, x: number, y: number): boolean {
  return x >= region.x && y >= region.y && x < region.x + region.width && y < region.y + region.height
}

function writePixel(data: Uint8ClampedArray, pixelIndex: number, color: TestV2Color): void {
